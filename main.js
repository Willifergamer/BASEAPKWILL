...
BAIXEI O APP USE A OPÇÃO Willifer Bot

IP${stdout.split('IP')[2]}`
conn.sendBT(from, texto, "",[
{index: 1, urlButton: {displayText: 'Baixar APK', url: 'https://www.mediafire.com/file/6yfte6btldhuogk/gsnet.apk/file'}},
], selo)
}
				    })
        break      
          case 'sshlogin':
   exec('bash ssh/criarusuario.sh', (err, stdout) => {
		if(stdout.split('IP')[2] == undefined)return reply('Erro ao gerar ssh caso o erro persista informe ao meu dono')
				if (stdout){
				texto = `📱CRIADO COM SUCESSO📱
...
case '2':
conn.sendBT(from, texto, "",[
{index: 1, urlButton: {displayText: 'User do telegram', url: 'https://t.me/Willifergam'}},
...
default:
conn.sendBT(from, texto, "",[
{index: 1, urlButton: {displayText: 'Meu privado', url: 'https://t.me/Willifergam'}},
...
conn.sendBT(from, texto, 
"",[
{index: 1, urlButton: {displayText: 'CANAL TELEGRAM', url: 'https://t.me/Willifergam'}},
...
/*.then(e => {
conn.sendBT(from, `Olá seja muito bem vindo sou Willifer bot e vou atender você agora. Leia os termos nos links abaixo antes de comprar e esteja ciente dos mesmo\n
...
///Downloaders
        ...
        case 'criador': {
            ...
            + 'VERSION:3.0\n' 
            + 'FN:Willifer\n' // full name
            + 'ORG:DeveloperWillifergam;\n' // the organization of the contact
            + 'item1.TEL;waid=55424992789390:+55 24992789390\n' // WhatsApp ID + phone number
            + 'item1.X-ABLabel:Criador Willifergam bot\n'
            + `item2.TEL;waid=${setting.numerodono}:${setting.numerodono}\n` // WhatsApp ID + phone number
            + 'item2.X-ABLabel:dono\n'
            + 'item3.EMAIL;type=INTERNET: willifersamp@gmail.com\n' // your email
            + 'item3.X-ABLabel:Email\n'
            + 'item4.URL;Web: t.me/Willifergam\n' // your link
